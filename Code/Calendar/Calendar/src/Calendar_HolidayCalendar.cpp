...
namespace Calendar
{
    ...
    boost::shared_ptr<HolidayCalendar> HolidayCalendar::Empty = boost::make_shared<HolidayCalendar>();
    ...
    boost::shared_ptr<WeekendCalendar> WeekendCalendar::Inst;
    ...
    void CompositeCalendar::AddCalendar(boost::shared_ptr<HolidayCalendarBase> pCalendar)
    {
        m_pCalendars.push_back(pCalendar);
    }

    bool CompositeCalendar::IsHoliday(Date date) const
    {
        BOOST_FOREACH(boost::shared_ptr<HolidayCalendarBase> pCalendar, m_pCalendars)
        {
            if (pCalendar->IsHoliday(date))
                return true;
        }
        return false;
    }
